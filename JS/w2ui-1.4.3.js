...
(function () {
    var w2grid = function(options) {
        ...
        this.last = {
            field     : 'all',
            caption   : w2utils.lang('Todos los campos'),
            logic     : 'OR',
            search    : '',
            searchIds : [],
            selection : {
    ...
    w2grid.prototype = {
        ...
        msgDelete       : w2utils.lang('&#191Est&#225 seguro de que desea borrar los registros seleccionados?'),
        msgNotJSON      : w2utils.lang('Los datos devueltos no está en formato JSON válida.'),
        msgAJAXerror    : w2utils.lang('Error de Ajax. Ver consola para obtener más detalles.'),
        ...
        buttons: {
            ...
            'search-go': { type: 'check',  id: 'w2ui-search-advanced', caption: w2utils.lang('B&uacutesqueda avanzada...'), hint: w2utils.lang('Busqueda avanzada...') },
        ...
        searchShowFields: function () {
            ...
            for (var s = -1; s < this.searches.length; s++) {
                ...
                if (s == -1) {
                    ...
                    search = { field: 'all', caption: w2utils.lang('Todos los campos') };
                } else {
        ...
        initColumnOnOff: function () {
            ...
            col_html += '<tr><td colspan="2" onclick="w2ui[\''+ obj.name +'\'].stateSave(); $(\'#w2ui-overlay\')[0].hide();">'+
                        ...
                        '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('Restaurar a valores por defecto') + '</div>'+
                        '</td></tr>';
            col_html += "</table></div>";
            ...
            if (typeof this.toolbar['render'] == 'undefined') {
                ...
                if (this.show.toolbarSearch) {
                    var html =
                        ...
                        '            onkeyUp="if (event.keyCode == 13 && w2utils.isIE){  this.onchange(); }"'+
                        '            onchange="'+
                        '                var val = this.value; '+
        ...
        getSearchesHTML: function () {
            ...
            html += '<tr>'+
                    ...
                    '        <div>'+
                    '        <button class="btn" onclick="obj = w2ui[\''+ this.name +'\']; if (obj) { obj.searchReset(); }">'+ w2utils.lang('Borrar') + '</button>'+
                    '        <button class="btn btn-blue" onclick="obj = w2ui[\''+ this.name +'\']; if (obj) { obj.search(); }">'+ w2utils.lang('Buscar') + '</button>'+
                    '        </div>'+
                    '    </td>'+
                    '</tr></table>';
            return html;
        ...
        updateOverlay: function () {
            ...
            if (['list', 'combo', 'enum'].indexOf(this.type) != -1) {
                ...
                if ($(input).is(':focus')) {
                    ...
                    $(el).w2menu('Actualizar', $.extend(true, {}, options, {
                        search     : false,
        ...
        validate: function (showErrors) {
            ...
            for (var f in this.fields) {
                ...
                switch (field.type) {
                    ...
                    case 'email':
                        if (this.record[field.name] && !w2utils.isEmail(this.record[field.name])) {
                            errors.push({ field: field, error: w2utils.lang('E-mail no valido') });
                        }
                        break;
                    case 'checkbox':
                        // convert true/false
                ...
                if (field.required && (val === '' || ($.isArray(val) && val.length == 0) || ($.isPlainObject(val) && $.isEmptyObject(val)))) {
                    errors.push({ field: field, error: w2utils.lang('Campo Requerido') });
                }
